...
PHP와 MySQL을 기반으로 구현한 **웹 채팅 서비스**입니다.  
AJAX 비동기 통신을 사용해 **페이지 새로고침 없이 실시간 메시지 송수신**이 가능하도록 구현했습니다.

회원가입 → 로그인 → 유저 목록 → 1:1 채팅까지  
웹 채팅 서비스의 **기본적인 흐름을 직접 구현**하는 것을 목표로 한 개인 프로젝트입니다.
...
서버는 **클라이언트 입력을 절대 신뢰하지 않고 검증해야 한다는 점**과 효율적인 **DB 설계 및 조회 패턴의 중요성**을 체감할 수 있었습니다.

현재는 AJAX 기반의 주기적 요청 방식으로 동작하지만,  
추후 **WebSocket 기반 실시간 통신**으로 전환하여 더 자연스러운 실시간성을 구현해볼 계획입니다.
